...
def test_job_library_migrates_legacy_db_and_keeps_data(tmp_path):
    ...
    # {43} (alignment_metrics + application_result, quality attribution)、
    # {44} (deadline) 与 {45} (usable_diffs, #111/ADR-0041) follow.
    assert _migrated_versions(store) == set(range(1, 29)) | {30, 35, 37, 38, 39, 42, 43, 44, 45}
...
def test_job_registry_inherits_shared_store(tmp_path):
    ...
    # 3 = ticket #101 jobs.request_id 加列（request_id 贯穿）
