...
def test_appraisal_uses_settings_benchmark_with_normalized_city():
    with patch("resualign.api._classify_job", return_value={}):
        client.put(
            "/api/settings",
            json={
        ...
        job = _create_appraisal_job(location="北京市朝阳区")

    started = time.monotonic()
    r = client.get(f"/api/jobs/{job['job_id']}/appraisal")
    elapsed = time.monotonic() - started
    assert r.status_code == 200
    body = r.json()
    assert body["benchmark_source"] == "设置表（城市）"
    assert body["city_normalized"] == "北京"
    assert body["salary_benchmark"] == 25000
    assert any("设置表（城市）" in reason for reason in body["reasons"])
    for key in ("score", "verdict", "components", "reasons", "weights"):
        assert key in body
    assert elapsed < 1.0


def test_appraisal_falls_back_to_library_median():
    with patch("resualign.api._classify_job", return_value={}):
        first = _create_appraisal_job(
            location="杭州",
            salary_min=20000,
            salary_max=25000,
            jd_text="Python backend A",
        )
        _create_appraisal_job(
            location="杭州",
            salary_min=30000,
            salary_max=35000,
            jd_text="Python backend B",
        )

    r = client.get(f"/api/jobs/{first['job_id']}/appraisal")
    assert r.status_code == 200
    body = r.json()
    assert body["benchmark_source"] == "库内同类中位"
    assert body["salary_benchmark"] == 25000
    assert body["city_normalized"] == "杭州"


def test_appraisal_neutral_when_no_benchmark():
    with patch("resualign.api._classify_job", return_value={}):
        job = _create_appraisal_job(
            location=None,
            job_function="其他",
            salary_min=None,
            salary_max=None,
    ...
    assert r.status_code == 200
    body = r.json()
    assert body["benchmark_source"] == "暂无基准"
