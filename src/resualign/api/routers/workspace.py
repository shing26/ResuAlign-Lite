...
def _session_or_404(session_id: str, user: dict[str, Any]) -> dict[str, Any]:
    session = context._session_store.get(session_id, tenant_id=user["user_id"])
    if session is None:
...
def init_workbench_session(
    ...
    existing = context._session_store.find_by_idempotency(
        user["user_id"], req.idempotency_key
    )
    if existing is not None:
        return context._workbench_service.public_state(existing)
    ...
    if raw_jd:
        try:
            job = context._workbench_service._create_library_job_without_llm(
                user,
            ...
            if "Duplicate job" in str(exc):
                existing = context._jobs.find_by_dedupe_key(
                    user["user_id"],
                    context._workbench_service._library_dedupe_key(raw_jd),
                )
                if existing is not None:
                    ...
                    if session is not None:
                        return context._workbench_service.public_state(
                            session
                        )
                    sections = (
                        context._workbench_service._session_sections_from_job(
                            existing
                        )
                    )
                    return context._workbench_service.public_state(
                        context._session_store.create(
                            user["user_id"],
                            ...
                            resume={
                                ...
                                "available_resumes": context._workbench_service._available_resumes(
                                    user["user_id"]
                                ),
                                "content_ref": None,
                            },
    ...
    session = context._session_store.create(
        user["user_id"],
    ...
    try:
        start_pipeline = context.build_config().is_llm_configured
    except Exception:
        start_pipeline = False
    if start_pipeline:
        context.enforce_daily_llm_cap(user["user_id"])
        threading.Thread(
            target=context._workbench_service._run_session_pipeline,
            args=(session["session_id"],),
            daemon=True,
        ).start()
    return context._workbench_service.public_state(session)
...
def get_workbench_session(
    ...
    state = context._workbench_service.public_state(session)
...
def get_workspace_session(
    ...
    if session is None:
        sections = (
            context._workbench_service._session_sections_from_job(job)
        )
        session = context._session_store.create(
            user["user_id"],
...
def analyze_workbench_session(
    ...
    return context._workbench_service.public_state(
        context._session_store.get(session_id)
    )
...
async def stream_workbench_events(
    ...
    cursor = context._session_store.events_cursor(
