...
POLISH_PROMPT = (
    "你是资深简历润色专家。请针对下面给出的单条「项目/工作经历」条目，"
    "用 STAR 法则（情境-任务-行动-结果）润色为更专业、更有说服力的表述。\n"
    "硬性规则：\n"
    "- 必须保留该条目的项目名称、时间、公司/团队/角色等事实行，不得改动或删除；\n"
    "- 严禁编造数据、指标或结果；只能保留原文已有的量化数字并优化表述，缺量化的地方"
    "不得凭空补数字，可以用更具体的行为描述替代；\n"
    "- 每条要点以行为动词开头，突出职责、技术难点、你的行动与结果；\n"
    "- 如果提供了「目标 JD」，把最相关 JD 关键词与业务场景自然融入表述；\n"
    "- 不要虚构姓名、链接或项目名；保持原文的 Markdown 列表风格（- 开头）。\n"
    '只返回 JSON：{"optimized": "润色后的整段内容（含标题行）", '
    '"rationale": "一句中文，说明改了什么、为什么更契合 HR/ATS"}。\n'
