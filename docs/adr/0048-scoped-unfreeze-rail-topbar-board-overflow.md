# ADR-0048: 冻结令 6e 逐项例外 —— 侧栏 / 顶栏 / 看板溢出

- **状态**: 已采纳
- **日期**: 2026-09-18
- **依据**: ADR-0042 决定 6e（冻结令例外流程）；ADR-0044 决定 10（token 命名与
  `--rail-w` / `--topbar-h` 取值）；施工单 `docs/plans/css-architecture-refactor-plan-2026-09-18.md`
- **关系**: 不推翻任何 ADR；本 ADR 只登记本轮冻结期内的三项例外及其边界

## 背景

ADR-0042 决定 6e 规定：自用线推进期间 app 冻结令继续有效，任何改动须逐项走
冻结令例外流程并留记录。本 ADR 即该流程的落地记录。

申请例外的动机是三条**自用闭环的直接摩擦**，不是架构动机：

1. 岗位库看板在 1706×780 视口下横向溢出 224px，第 5 列被裁，每轮投放操作都要
   横向滚动才能看到完整列；
2. `styles.css` 里 `.app-rail` / `.topbar` 有多份互相覆盖的 240px / 230px / 64px
   定义，实测生效值与设计定稿（224px / 52px）不一致，且「改一处不生效」本身
   持续消耗调试时间；
3. `alignment-gap.test.mjs` 锁的是被覆盖的死定义（240px / 64px），永远绿灯，
   无法作为护栏。

改造前实测基线（Playwright，1706×780，DPR 1，8 路由 0 console error）：

| 指标 | 实测 |
|---|---|
| `.app-rail.rail` 宽 | 230px |
| `.topbar` 高 | 64px |
| `#job-board` 可视宽 / 内容宽 | 1424px / 1648px（溢出 224px） |
| `.board-column` flex 简写 | `0 0 320px`（`styles.css:8922`） |

## 决定

**决定 1（范围）**：本轮只做以下三项，任何第四项改动都须另开例外：

1. `.app-rail.rail` 与 `.app-shell` 桌面端宽度：230px → **224px**，并统一引用
   `--rail-w`；
2. `.topbar` 高度：64px → **52px**，并统一引用 `--topbar-h`；
3. `.board-column` 弹性：`flex: 0 0 320px` → **可收缩弹性列**，使 5 列在
   1424px 可视宽内不被裁切。

**决定 2（token 落点）**：`--rail-w: 224px` 与 `--topbar-h: 52px` 本批落在
`styles.css` 顶部既有基础 `:root` 块内，作为 B1 `@layer tokens` 的前置形态；
不新增第二个 `<link>`、不新增 `design-tokens.css` 文件。取值与 ADR-0044 决定 10
完全一致。

**决定 3（清理死定义）**：本批同时把 `.app-rail` / `.topbar` 的旧魔法数字改写为
token 引用，并清掉被后文覆盖的无效 token 定义，避免留下「声明了但不生效」的
第二处验证死定义。

**决定 4（测试同步）**：`alignment-gap.test.mjs` 的两条魔法数字断言改为「必须引用
`--rail-w` / `--topbar-h`」；`adr0033.test.mjs` 的静态版本号 v=33 → v=34。测试
先改，再改 CSS。

**决定 5（不做）**：本轮不引入 `@layer`、不做 token 全量收敛、不删 100 处
`!important`、不做字体自托管、不引入构建工具。B0–B7 施工单的其余内容保持
「解冻后执行」。

### 第二批：看板窄屏与卡片密度（2026-09-18 追加，同一 6e 例外内）

**决定 6（列宽保底）**：`.board-column` 从 `flex: 1 1 200px` 改为
`flex: 1 1 220px` + `min-width: 220px`。列不再被压到不可读：中间档视口下
宁可让 `.board` 横向滚动，也不把卡片挤成 122px 宽。

**决定 7（四维按容器实宽降级）**：`.board-match` 设为
`container-type: inline-size`；当容器实宽 ≤ 419px 时，`.board-match__dims`
从四列改为 2×2，且 `.match-dim` 改为「标签块独占一行 + 轨道占满下一行」。
理由是实测四维并排在 40.5px/列时会把中文标签拆成 5 行竖排、把
`.match-dim__track` 压成 0px；按视口宽度猜断点不可靠，必须按卡片实宽判定。
≥1280px 视口下卡片仍走四列一行，保持紧凑。

**决定 8（不做卡片信息删减）**：卡片内容（匹配四维、理由、标签、时间线、
投递按钮、状态下拉）都有独立用途，本轮不隐藏、不折叠、不引入新的交互控件；
密度问题通过列宽与布局自适应解决，而不是砍信息。

### 第三批：工作台取证与例外收口（2026-09-18 追加）

**背景**：第二批发版后继续取证工作台，判断是否还有值得走 6e 的 CSS 缺陷。

**取证结果（`.scratch/workspace_probe.py`，8 档视口）**：

| 视口 | `.wb-grid` | 左栏 / 右栏 | 横向溢出 | console error |
|---|---|---|---|---|
| 390–768 | 单栏 | 全宽 | 无 | 0 |
| 1024 | `476px 320px` | 319 / 476 | 无 | 0 |
| 1280 | `647px 405px` | 404 / 647 | 无 | 0 |
| 1440 | `746px 466px` | 465 / 746 | 无 | 0 |
| 1706 | `910px 568px` | 568 / 910 | 无 | 0 |
| 1920 | `1041px 651px` | 650 / 1041 | 无 | 0 |

九张 diff 卡在右栏正常渲染，≤768 降级为单栏、≥1024 双栏，无裁切。
失败态横幅、重跑入口、移动端底部动作条均由既有守卫测试覆盖，本轮取证未发现
新的结构性缺陷。

**决定 9（例外路线收口）**：本轮到此关闭 6e 例外，不再继续切 CSS 批次。

理由：

1. 看板列宽已保底 220px 且有横向滚动，四维可读，列不再被压到不可用；
2. 工作台在 8 档视口下结构健康，没有可量化的布局缺陷；
3. 剩余摩擦（对齐失败态的信息表达、卡片信息优先级、`no_output` 的复跑引导）
   属**产品逻辑与数据层**问题，不是 CSS 能解决的；
4. 其余结构性改造（`@layer`、token 全量收敛、`!important` 归零、死代码清理）
   是 B1–B7 整单内容，按 ADR-0042 决定 6e 找不到「让自用闭环好用」的正当理由，
   应等自用线到 ≥3 例或探针线出结果后统一解冻。

**留给下一阶段的问题（不属于本 ADR 范围，仅登记）**：

- 对齐失败态在部分入口只以文案出现，复跑入口的可见性待产品侧确认；
- 卡片信息密度若仍不满意，应走「信息折叠」产品决策，而不是重新压窄列宽；
- `no_output` 假成功主症状的归因见 issue #110 / 派单 #115。

## 验收

- DOM 度量：`.app-rail.rail` = 224px、`.topbar` = 52px、`#job-board` 溢出 = 0、
  5 列等宽且第 5 列右边界不超出视口。
- 窄屏度量（`.scratch/board-responsive.json`）：列宽恒 ≥ 220px；
  1024 视口下列宽 220（原 122）、卡片高 495.5（原 626）。
- 四维度量（`.scratch/match_dim_probe.py`）：标签单行不拆字、轨道宽度 = 容器宽
  （原 0px），1024 / 1440 视口均通过。
- 8 条路由 0 console error；`node --test` 前端全绿；后端回归基线不变
  （997 passed / 7 skipped）。
- 探针与前后快照归档 `.scratch/css-metrics-*.json`，不入库。

## 后果

- 自用闭环的看板操作不再需要横向滚动才能看全 5 列；字号、间距、配色、图标、
  层序均未触碰，视觉改动限于侧栏 −6px、顶栏 −12px、看板列由 320px 缩至
  275px 左右。
- 本轮为 B1 预置了 `--rail-w` / `--topbar-h` 两个 canonical token，B1 施工时
  只需改 token 值所在层，不必再追魔法数字。
- 中间档视口（≤1440）卡片比之前高约 105px，换取四维可读与列宽可用；这是
  有意的取舍，不是回归。若自用阶段发现卡片过高，下一步应做「信息折叠」
  决策而不是重新压窄列宽。
- 未解冻其余前端架构改造；B0–B7 仍然整单待施工。
