...
- 计数口径：棘轮的 `api_module._` 只统计**代码站点**，注释里的提及不计
  （当前代码 332 处，另有 1 处出现在 `api/__init__.py` 的注释里）。
...
| 项 | 状态 | 提交 |
| --- | --- | --- |
| R1 / R2 组合根 | 完成 | `274cbb4` |
| R3-A `update_job` | 完成 | `d14769d` |
| R3-B `_run_job_holding_gate` | 完成 | `95a8593` |
| 配置 fail-fast（R5 / 共性 2） | 完成 | `0df6a18` |
| 扩展点注册表（共性 4） | 完成 | `a48a73f` |
| R4 残余（删文本回退） | 完成 | `109d641` |
| R7 目录分包 | 引擎批完成；其余批未做 | `a9515be` |

验收证据：后端 `1138 passed / 7 skipped`；前端 `531 passed`；E2E `7 passed`；
`ruff` 全仓通过；`modgraph --check` 违规 0 / 循环 0 / `api_module._` 0。

**R7 未完成批的处置（2026-09-21）**

- 引擎批已按本施工单落地：`engine.py` 转包，`llm` / `llm_nodes` /
  `llm_providers` / `role_router` / `tailor` 移入 `engine/`，全仓导入与
  modgraph 层表同步。
- `domain/` 与 `storage/` 两批**暂停**，原因是收益与风险不匹配：两批合计
  约 23 个模块、上百处导入，且 `storage/` 与 `engine/` 存在双向依赖
  （`llm_nodes -> secret_box/store_base`、`engine -> domain`），搬迁后
  仍需保留大量兼容路径才能不破外部引用，属于纯组织性改动。
- 恢复条件：出现真实的导航/归属痛点（例如新增第二个同类领域包、或新人
  反复找错模块）时，再按「一次一包 + 全量测试 + modgraph 收紧」的方式
  重启；重启前先补一个与引擎批同款的通用重写器，并为其写回归测试。
- 该暂停不阻塞其余挂起批——R7 是最后一项，且不改变运行时行为。
