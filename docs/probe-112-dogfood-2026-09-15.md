...
> **勘误（2026-09-16，ADR-0042）**：§末行「判据时钟 2026-09-15 → 2026-10-13」
> 的**起点与口径已被重锚**——时钟起点改为**四渠道 outreach 全部发出当日**，
> 09-23 24:00 前未发完则登记 `dist_not_executed`（时钟不启动、非证伪）。
> 三档数值阈值一字未改。本文档正文保留 09-15 当时记录，不改史；
> 执行口径见 `docs/probe-112-send-checklist-2026-09-16.md`。
