...
边界待锐化（2026-09-14 必要性审查）：现定义「一句自然语言指令驱动既有确定性流程」同样适用于**预设流**，缺的正是把两者分开的唯一特征——执行期由模型逐步决定下一步调用（决策循环）。ADR-0040 若生效，此定义须补该特征。
...
**三线门槛 (Three Gates)**
Phase A 开工前的三道灯：能力线（Phase 0 过线）、地基线（对齐成功率回线 + 零 diff 假成功被 CI 锁死）、需求线（同一简历 ≥3 轮改→评→再改 ≥N 例 ∧ 非测试租户周活 ≥M 且连续 4 周不中断）。三线全亮才开工；Phase 0 过线只点亮能力线。见 ADR-0040（proposed）。
_Avoid_: 用 Phase 0 过线当三线总和；把「未上线所以验不了」当成需求线的 false 理由
